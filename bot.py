...
STATUSES = {
    0: {"name": "🟢 Обычный", "mult": 1.00},
    1: {"name": "🟡 Продвинутый", "mult": 1.05},
    2: {"name": "🔴 VIP", "mult": 1.10},
    3: {"name": "🟣 Премиум", "mult": 1.15},
    4: {"name": "👑 Легенда", "mult": 1.25},
    5: {"name": "⚡ Создатель", "mult": 1.50},
}
...
def parse_amount(text: str) -> float:
    ...
    mul = 1.0
    if raw.endswith("ккккк"): mul, raw = 100_000_000_000_000, raw[:-5]
    elif raw.endswith("кккк"): mul, raw = 1_000_000_000_000, raw[:-4]
    elif raw.endswith("ккк"): mul, raw = 1_000_000_000, raw[:-3]
    elif raw.endswith("кк"): mul, raw = 1_000_000, raw[:-2]
    elif raw.endswith("к"): mul, raw = 1_000, raw[:-1]
    elif raw.endswith("b"): mul, raw = 1_000_000_000, raw[:-1]
    elif raw.endswith("m"): mul, raw = 1_000_000, raw[:-1]
    return round(float(raw) * mul, 2) if raw else 0

def fmt_amount(v: float) -> str:
    if v >= 1_000_000_000_000: return f"{v/1_000_000_000_000:.2f}ккккк"
    if v >= 1_000_000_000: return f"{v/1_000_000_000:.2f}ккк"
    if v >= 1_000_000: return f"{v/1_000_000:.2f}кк"
    if v >= 1_000: return f"{v/1_000:.2f}к"
...
def log_admin(admin_id: int, act: str, target: str = None, det: str = ""):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("INSERT INTO admin_logs (admin_id,action,target,details,ts) VALUES (?,?,?,?,?)",
              (str(admin_id), act, target, det, now_ts()))
    conn.commit()
    conn.close()
...
def ban_user(uid: int):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("UPDATE users SET is_banned = 1 WHERE id = ?", (str(uid),))
    conn.commit()
    conn.close()
...
def unban_user(uid: int):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("UPDATE users SET is_banned = 0 WHERE id = ?", (str(uid),))
    conn.commit()
    conn.close()
...
def claim_bonus(uid: int):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT last_claim, streak FROM daily WHERE user_id = ?", (str(uid),))
    row = c.fetchone()
    ...
    c.execute("INSERT OR REPLACE INTO daily (user_id, last_claim, streak) VALUES (?,?,?)",
              (str(uid), now, streak))
    conn.commit()
    conn.close()
...
def claim_check(uid: int, code: str):
    ...
    if row[3] <= 0:
        conn.close(); return False, "Использован", 0
    claimed = json.loads(row[4])
    if str(uid) in claimed:
        conn.close(); return False, "Уже активирован", 0
...
def claim_promo(uid: int, code: str):
    ...
    row = c.fetchone()
    if not row:
        conn.close(); return False, "Не найден", 0
    if row[2] <= 0:
        conn.close(); return False, "Закончился", 0
...
def open_deposit(uid: int, amount: float, term: int):
    ...
    conn.commit()
    conn.close()
    return True, f"Депозит {term} дней открыт"
...
def roulette_spin(choice: str):
    ...
    col = "red" if num in RED_NUMBERS else "black" if num != 0 else "green"
    win, mult = False, 0
...
def share_check_kb(code: str, amount: float):
    share = f"https://t.me/share/url?url=Чек {code} на {fmt_money(amount)}! Забери в @onemi_bot"
    return InlineKeyboardMarkup(inline_keyboard=[
...
async def start_cmd(m: Message):
    ...
    await m.answer(f"🎮 ONEmi Game Bot\n👤 {u['name']}\n💰 {fmt_money(u['balance'])}", reply_markup=main_kb())
...
async def crash_text(m: Message):
    ...
    if u["balance"] < bet:
        await m.answer("❌ Не хватает")
        return
    update_balance(m.from_user.id, -bet)
    point = crash_roll()
    if target <= point:
        win = bet * target * get_mult(m.from_user.id)
        update_balance(m.from_user.id, win)
        add_bet(m.from_user.id, "crash", bet, win, target)
...
async def cube_text(m: Message):
    ...
    res = roll_dice()
    if guess == res:
        win = bet * 5.8 * get_mult(m.from_user.id)
        update_balance(m.from_user.id, win)
        add_bet(m.from_user.id, "cube", bet, win, 5.8)
...
async def dice_text(m: Message):
    parts = m.text.lower().split()
    if len(parts) != 3:
        await m.answer("❌ кости 10к м|б|равно")
        return
    try:
        bet = parse_amount(parts[1])
    except:
...
async def football_text(m: Message):
    parts = m.text.lower().split()
    if len(parts) != 3:
        await m.answer("❌ футбол 10к гол|мимо")
        return
    try:
        bet = parse_amount(parts[1])
    except:
    ...
    if win:
        win_amount = bet * 1.85 * get_mult(m.from_user.id)
        update_balance(m.from_user.id, win_amount)
...
async def basket_text(m: Message):
    parts = m.text.lower().split()
    if len(parts) != 3:
        await m.answer("❌ баскет 10к попал|промах")
        return
    try:
        bet = parse_amount(parts[1])
    except:
    ...
    if win:
        win_amount = bet * 2.2 * get_mult(m.from_user.id)
        update_balance(m.from_user.id, win_amount)
...
async def cb_top(c: CallbackQuery):
    ...
    if not top:
        ...
        for i, u in enumerate(top,1):
            ...
            text += f"{medal} {STATUSES[u['status']]['name'].split()[0]} {u['name']} — {fmt_money(u['balance'])}\n"
...
async def cb_profile(c: CallbackQuery):
    ...
    await c.message.answer(
        f"👤 {u['name']}\n"
        f"🎭 {STATUSES[u['status']]['name']} (x{STATUSES[u['status']]['mult']})\n"
...
async def cb_bank(c: CallbackQuery):
    await c.message.edit_text("🏦 Банк депозитов", reply_markup=bank_kb())
    await c.answer()
...
async def cb_transfer(c: CallbackQuery, state: FSMContext):
    ...
    await c.message.answer("💸 Введи ID или @username получателя:")
    await c.answer()
...
async def transfer_target(m: Message, state: FSMContext):
    ...
    await m.answer(f"Введи сумму (макс {fmt_money(TRANSFER_MAX)}):")
...
async def transfer_amount(m: Message, state: FSMContext):
    try:
        ...
        return
    if amount <= 0 or amount > TRANSFER_MAX:
        await m.answer(f"❌ 0..{fmt_money(TRANSFER_MAX)}")
        return
...
async def cb_help(c: CallbackQuery):
    await c.message.answer(
        "❓ Помощь\n\n"
        "🎮 Игры:\nкраш 100к 4 | кубик 10к 5 | кости 50к м|б|равно\n"
        "футбол 100к гол|мимо | баскет 100к попал|промах\n\n"
        "💸 Перевод: /transfer @ник\n"
        "🎁 Бонус: /bonus\n"
        "👑 Админ: /admin"
    )
    await c.answer()
...
async def game_bet(m: Message, state: FSMContext):
    try:
        ...
        if bet < MIN_BET:
            await m.answer(f"❌ Мин {fmt_money(MIN_BET)}")
            return
    except:
        await m.answer("❌ Сумма")
        return
    ...
    if game == "roulette":
        ...
        await state.set_state(BetState.crash_target)
        await m.answer(f"💰 {fmt_money(bet)}\nМножитель:", reply_markup=crash_kb())
    elif game == "cube":
        await m.answer(f"💰 {fmt_money(bet)}\nЧисло:", reply_markup=cube_kb())
    elif game == "dice":
        await m.answer(f"💰 {fmt_money(bet)}\nУсловие:", reply_markup=dice_kb())
    elif game == "football":
        await m.answer(f"💰 {fmt_money(bet)}\nИсход:", reply_markup=football_kb())
    elif game == "basket":
        await m.answer(f"💰 {fmt_money(bet)}\nИсход:", reply_markup=basket_kb())
    elif game == "tower":
        await state.clear()
        update_balance(m.from_user.id, -bet)
        ...
        await state.update_data(bet=bet)
        await state.set_state(BetState.mines_count)
        await m.answer(f"💰 {fmt_money(bet)}\nСколько мин? (1-5):")
...
async def roulette_play(c: CallbackQuery, state: FSMContext):
    ...
    if win:
        win_amount = bet * mult * get_mult(uid)
...
@dp.callback_query(BetState.crash_target, F.data.startswith("cr:"))
async def crash_play(c: CallbackQuery, state: FSMContext):
    ...
    point = crash_roll()
    if target <= point:
        win = bet * target * get_mult(uid)
        update_balance(uid, win)
        add_bet(uid, "crash", bet, win, target)
        text = f"📈 Краш x{point}\n✅ +{fmt_money(win)}"
    else:
...
async def cube_play(c: CallbackQuery, state: FSMContext):
    ...
    res = roll_dice()
    if guess == res:
        win = bet * 5.8 * get_mult(uid)
        update_balance(uid, win)
        add_bet(uid, "cube", bet, win, 5.8)
        text = f"🎲 {res}\n✅ +{fmt_money(win)}"
    else:
...
async def football_play(c: CallbackQuery, state: FSMContext):
    ...
    if win:
        win_amount = bet * 1.85 * get_mult(uid)
        update_balance(uid, win_amount)
...
async def basket_play(c: CallbackQuery, state: FSMContext):
    ...
    if win:
        win_amount = bet * 2.2 * get_mult(uid)
        update_balance(uid, win_amount)
...
async def tower_play(c: CallbackQuery):
    ...
    if not game:
        await c.answer("Нет игры")
        return
    if act == "cash":
        if game["level"] == 0:
            win_amount = game["bet"]
        else:
            win_amount = game["bet"] * TOWER_MULT[game["level"]-1] * get_mult(uid)
        update_balance(uid, win_amount)
        ...
        if choice == safe:
            ...
            if game["level"] >= len(TOWER_MULT):
                win_amount = game["bet"] * TOWER_MULT[-1] * get_mult(uid)
                update_balance(uid, win_amount)
...
async def diamond_play(c: CallbackQuery):
    ...
    if act == "cash":
        if game["level"] == 0:
            ...
            win_amount = game["bet"] * DIAMOND_MULT[game["level"]-1] * get_mult(uid)
        update_balance(uid, win_amount)
        ...
        if choice == safe:
            ...
            if game["level"] >= len(DIAMOND_MULT):
                win_amount = game["bet"] * DIAMOND_MULT[-1] * get_mult(uid)
                update_balance(uid, win_amount)
...
async def gold_play(c: CallbackQuery):
    ...
    if act == "cash":
        if game["level"] == 0:
            ...
            win_amount = game["bet"] * GOLD_MULT[game["level"]-1] * get_mult(uid)
        update_balance(uid, win_amount)
        ...
        if choice == safe:
            ...
            if game["level"] >= len(GOLD_MULT):
                win_amount = game["bet"] * GOLD_MULT[-1] * get_mult(uid)
                update_balance(uid, win_amount)
...
@dp.message(BetState.mines_count)
...
async def mines_action(c: CallbackQuery):
    ...
    if act == "cash":
        if len(game.opened) == 0:
            ...
            return
        win = game.bet * game.get_mult() * get_mult(uid)
        update_balance(uid, win)
        add_bet(uid, "mines", game.bet, win, game.get_mult())
        del active_mines[uid]
        await c.message.answer(f"💣 Забрано {fmt_money(win)}\n💰 {fmt_money(get_user(uid)['balance'])}", reply_markup=play_again_kb("mines"))
    elif act == "noop":
        await c.answer()
        ...
        if not win:
            ...
            if len(game.opened) >= (9 - game.mines):
                win_amount = game.bet * mult * get_mult(uid)
                update_balance(uid, win_amount)
...
async def blackjack_bet(m: Message, state: FSMContext):
    ...
    if pv == 21:
        if dv == 21:
            ...
            win = bet * 2.5 * get_mult(m.from_user.id)
            update_balance(m.from_user.id, win)
            add_bet(m.from_user.id, "blackjack", bet, win, 2.5)
...
async def blackjack_action(c: CallbackQuery):
    ...
    if act == "hit":
        ...
        if dv > 21 or game["pv"] > dv:
            win = game["bet"] * 2 * get_mult(uid)
            update_balance(uid, win)
            add_bet(uid, "blackjack", game["bet"], win, 2)
...
async def adm_give(c: CallbackQuery, state: FSMContext):
    if not is_admin(c.from_user.id):
        await c.answer("⛔")
        return
    await state.set_state(AdminGiveState.target)
    await c.message.answer("💰 ID и сумма:\n123456789 100кк")
    await c.answer()

@dp.message(AdminGiveState.target)
...
async def adm_status(c: CallbackQuery, state: FSMContext):
    if not is_admin(c.from_user.id):
        await c.answer("⛔")
        return
    await state.set_state(AdminStatusState.target)
    await c.message.answer("👑 Введи ID пользователя:")
    await c.answer()

@dp.message(AdminStatusState.target)
...
async def adm_users(c: CallbackQuery):
    ...
    text = "👥 Игроки:\n"
    for u in users:
        text += f"{STATUSES[u['status']]['name'].split()[0]} {u['name']} — {fmt_money(u['balance'])}\n"
    await c.message.answer(text)
    await c.answer()
...
async def adm_broadcast_send(m: Message, state: FSMContext):
    ...
    for uid in users:
        try:
            await m.bot.send_message(int(uid[0]), f"📢 Рассылка\n\n{text}")
            sent += 1
...
async def adm_promos(c: CallbackQuery):
    if not is_admin(c.from_user.id):
        await c.answer("⛔")
        return
    await c.message.answer("🎟 Промокоды\n/addpromo КОД НАГРАДА АКТИВАЦИИ\nИли создай:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="➕ Создать", callback_data="adm:create_promo")],
        [InlineKeyboardButton(text="📋 Список", callback_data="adm:list_promos")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="adm:back")],
    ]))
    await c.answer()

@dp.callback_query(F.data == "adm:create_promo")
...
async def adm_ban(c: CallbackQuery, state: FSMContext):
    if not is_admin(c.from_user.id):
        await c.answer("⛔")
        return
    await state.set_state(AdminBanState.target)
    await c.message.answer("🚫 Введи ID для бана:")
    await c.answer()

@dp.message(AdminBanState.target)
